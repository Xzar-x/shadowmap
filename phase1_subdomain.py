...
def _execute_tool_command(
    ...
    try:
        ...
        if process.returncode == 0:
            ...
            utils.log_and_echo(
                f"Narzędzie {tool_name} zakończyło pracę z kodem błędu ({process.returncode}), ale kontynuuję.",
                "WARN",
            )
            return output_file
...
def start_phase1_scan() -> Tuple[Dict[str, str], List[Dict[str, Any]], List[str]]:
    ...
    utils.console.print(
        Align.center(
            f"[bold green]Rozpoczynam Fazę 1 - Odkrywanie Subdomen dla {config.ORIGINAL_TARGET}...[/bold green]"
        )
    )
    ...
    with Progress(
        ...
        task1_total = sum(
            1
            for i, enabled in enumerate(config.selected_phase1_tools)
            if enabled and not (config.TARGET_IS_IP and i < 3)
        )
        task1 = progress.add_task(
            "[green]Faza 1 (Subdomeny)[/green]",
            total=task1_total if task1_total > 0 else 1,
        )
        ...
        if config.SAFE_MODE:
            ...
            shuffled_wordlist_p1_path = utils.shuffle_wordlist(
        ...
        tool_configurations = [
            ...
            {
                "name": "Puredns",
                "cmd_template": puredns_base_cmd + ["--rate-limit", "1000", "-q"],
            },
        ]
        ...
        for i, tool_config in enumerate(tool_configurations):
            if config.selected_phase1_tools[i] == 1:
                if config.TARGET_IS_IP and tool_config["name"] in [
                    "Subfinder",
                    "Assetfinder",
                    "Findomain",
                ]:
                ...
                output_path = os.path.join(
                    config.REPORT_DIR, f"{tool_config['name'].lower()}_results.txt"
                )
                tasks_to_run.append(
                    (tool_config["name"], tool_config["cmd_template"], output_path)
                )
        ...
        if not tasks_to_run:
            utils.console.print(
                Align.center(
                    "Nie wybrano narzędzi do odkrywania subdomen. Pomijam.",
                    style="bold yellow",
                )
            )
            return {}, [], []
    ...
    unique_lines = sorted(
        list(
            set(
                ...
                if line.strip()
                and (
                    f".{config.CLEAN_DOMAIN_TARGET}" in line.lower()
    ...
    with utils.console.status(
        "[bold green]Weryfikuję subdomeny i pobieram nagłówki za pomocą HTTPX...[/bold green]"
    ):
        ...
        if config.CUSTOM_HEADER:
            ...
            httpx_command.extend(
                ["-H", f"User-Agent: {utils.get_random_user_agent_header()}"]
            )
    ...
    while True:
        ...
        for i, tool_name in enumerate(tool_names):
            ...
            if config.TARGET_IS_IP and i < 3:
                table.add_row(
                    f"[{i+1}]",
                    f"[dim]{status_char}[/dim] [dim]{tool_name} (pominięto dla IP)[/dim]",
                )
            else:
        ...
        if choice.isdigit() and 1 <= int(choice) <= 4:
            ...
            if config.TARGET_IS_IP and idx < 3:
                utils.console.print(
                    Align.center(
                        "[bold yellow]Nie można włączyć narzędzi pasywnych dla celu IP.[/bold yellow]"
                    )
                )
            else:
                config.selected_phase1_tools[idx] = (
                    1 - config.selected_phase1_tools[idx]
                )
        elif choice.lower() == "s":
            ...
            if any(config.selected_phase1_tools):
                ...
                utils.console.print(
                    Align.center(
                        "[bold yellow]Proszę wybrać co najmniej jedno narzędzie lub wrócić/wyjść.[/bold yellow]"
                    )
                )
        else:
            utils.console.print(
                Align.center(
                    "[bold yellow]Nieprawidłowa opcja. Spróbuj ponownie.[/bold yellow]"
                )
            )
        time.sleep(0.1)
