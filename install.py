    ...
    try:
        if live_output:
            ...
            if process.returncode != 0:
                console.print(Align.center(f"[red]Błąd[/red] podczas '{description}': Zwrócono kod {process.returncode}"))
                return False
        else:
            result = subprocess.run(full_command, check=True, capture_output=True, text=True)
        return True
    except FileNotFoundError:
        console.print(Align.center(f"[red]Błąd[/red] podczas '{description}': Polecenie '{full_command[0]}' nie zostało znalezione."))
        return False
    except subprocess.CalledProcessError as e:
        console.print(Align.center(f"[red]Błąd[/red] podczas '{description}': Polecenie zwróciło błąd (kod {e.returncode})."))
        console.print(Align.center(Panel(e.stderr, title="[red]STDERR[/red]", border_style="red", expand=False)))
        return False
...
def check_dependencies():
    ...
    console.print(Align.center(Columns([system_table, go_table], align="center", expand=True)))
    return missing_system, missing_go

def main():
    display_banner()
    console.print(Align.center(Panel.fit("[bold]Instalator ShadowMap sprawdzi i zainstaluje wszystkie wymagane zależności.[/bold]", border_style="green")))
    ...
    if not missing_system_deps and not missing_go_tools:
        ...
        if ASSUME_YES or NONINTERACTIVE:
            ...
            install_confirmed = questionary.confirm("Czy chcesz spróbować zainstalować brakujące pakiety?").ask()

        if install_confirmed:
            if missing_system_deps:
                console.print(Align.center(f"\n[blue]Instalowanie zależności systemowych: {', '.join(missing_system_deps)}...[/blue]"))
                run_command(["apt-get", "update"], "Aktualizacja listy pakietów", sudo=True)
                run_command(["apt-get", "install", "-y"] + missing_system_deps, "Instalacja pakietów systemowych", sudo=True, live_output=True)
            
            if missing_go_tools:
                console.print(Align.center(f"\n[blue]Instalowanie narzędzi Go: {', '.join(missing_go_tools)}...[/blue]"))
                for tool in missing_go_tools:
                    run_command(["go", "install", "-v", GO_TOOLS[tool]], f"Instalacja {tool}", live_output=True)

    console.print(Align.center(f"\n[blue]Instalowanie/aktualizowanie pakietów Python ({', '.join(PYTHON_PKGS)})...[/blue]"))
    ...
    run_command(["cp", os.path.join(base_dir, "shadowmap"), BIN_DIR], "Kopiowanie głównego skryptu", sudo=True)
    ...
    for f_name in files_to_copy_to_share:
        src = os.path.join(base_dir, f_name)
        if os.path.exists(src):
            run_command(["cp", src, SHARE_DIR], f"Kopiowanie zasobu: {f_name}", sudo=True)

    final_message = Panel(
        Text.from_markup("[bold green]Instalacja ShadowMap zakończona pomyślnie![/bold green]\n\n"
                         "Możesz teraz uruchomić narzędzie wpisując w terminalu: [bold cyan]shadowmap <cel>[/bold cyan]\n\n"
                         "[yellow]Uwaga:[/yellow] Może być konieczne ponowne uruchomienie terminala, aby zmiany w ścieżce PATH weszły w życie.", justify="center"),
