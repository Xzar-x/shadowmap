...
class UserAgentRotator:
    ...
    def __init__(self, user_agents_file: str, rotation_interval: int = 50):
        ...
        self.call_count = 0
        self.current_user_agent = random.choice(self.user_agents) if self.user_agents else "ShadowMap/1.0"
...
class WafHealthMonitor:
    ...
    def __init__(self, target_url: str):
        if not requests:
            raise ImportError("Biblioteka 'requests' jest wymagana do monitorowania WAF.")
    ...
    def _make_request(self, url: str) -> Optional[requests.Response]:
        try:
            ...
            return requests.get(url, headers=headers, timeout=10, verify=False, allow_redirects=False)
    ...
    def establish_baseline(self) -> bool:
        ...
        if not positive_res:
            log_and_echo("Health Check: Nie udało się połączyć z celem, aby ustalić linię bazową.", "WARN")
            return False
        ...
        if not negative_res:
            log_and_echo("Health Check: Nie udało się wykonać drugiego zapytania do linii bazowej.", "WARN")
            return False
    ...
    def _check_against_baseline(self):
        ...
        if current_positive.status_code != self.baseline["positive"]["status"]:
            log_and_echo(f"Health Check: WYKRYTO BLOKADĘ! Status strony głównej zmienił się z {self.baseline['positive']['status']} na {current_positive.status_code}", "WARN")
            self.is_blocked_event.set()
        
        # Bardziej zaawansowane: hash się zmienił, ale status jest ten sam (np. 200 OK ze stroną blokady)
        elif hashlib.md5(current_positive.content).hexdigest() != self.baseline["positive"]["hash"]:
            log_and_echo(f"Health Check: WYKRYTO BLOKADĘ! Hash strony głównej uległ zmianie przy statusie {current_positive.status_code}", "WARN")
            self.is_blocked_event.set()
    ...
    def run_monitor(self):
        ...
        while not self.stop_monitor_event.is_set():
            ...
            sleep_time = random.uniform(config.WAF_CHECK_INTERVAL_MIN, config.WAF_CHECK_INTERVAL_MAX)
