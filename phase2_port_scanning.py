...
def _run_scan_tool(
    ...
    if tool_name == "Masscan":
        ...
        if not ip_targets:
            utils.console.print(
                "[bold red]Brak poprawnych adresów IP dla Masscana![/bold red]"
            )
            return None
    ...
    # Rozwiąż pełną ścieżkę do pliku wykonywalnego (zwłaszcza dla narzędzi Go w ~/go/bin)
    exe_name = final_command[0]
    resolved_path = shutil.which(exe_name)
    if not resolved_path:
        real_home = os.path.expanduser("~")
        for extra_dir in [
            os.path.join(real_home, "go", "bin"),
            os.path.join(real_home, ".local", "bin"),
            "/usr/local/bin",
        ]:
            candidate = os.path.join(extra_dir, exe_name)
            if os.path.isfile(candidate) and os.access(candidate, os.X_OK):
                resolved_path = candidate
                break
    if resolved_path:
        final_command[0] = resolved_path

    sudo_prefix = []
    if tool_name == "Masscan" and os.geteuid() != 0:
        sudo_prefix = ["sudo"]
    elif tool_name == "Naabu":
        # Naabu działa bez roota z -s c (Connect scan); nie wymuszamy sudo
        if "-s" not in final_command and "-scan-type" not in final_command:
            final_command.extend(["-s", "c"])
    ...
    try:
        ...
        if process.returncode == 0:
            ...
            if stdout:
                utils.console.print(f"[dim]{stdout.strip()}[/dim]")
            Prompt.ask("\n[bold yellow]Wystąpił błąd narzędzia. Naciśnij Enter, aby kontynuować...[/bold yellow]", default="")
            return None
        ...
        Prompt.ask("\n[bold yellow]Limit czasu minął. Naciśnij Enter, aby kontynuować...[/bold yellow]", default="")
        return None
    except Exception as e:
        utils.console.print(f"[bold red]Wyjątek przy {tool_name}: {e}[/bold red]")
        Prompt.ask("\n[bold yellow]Wystąpił wyjątek. Naciśnij Enter, aby kontynuować...[/bold yellow]", default="")
        return None
    finally:
