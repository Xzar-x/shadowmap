        ...
        if res_file and os.path.exists(res_file):
            ...
            try:
                ...
                if ports_found_in_xml > 0:
                    utils.console.print(f"[green]Nmap XML: Zaktualizowano wyniki. Znaleziono {ports_found_in_xml} otwartych portów.[/green]")
                else:
                    utils.console.print(f"[yellow]Nmap XML: Przetworzono poprawnie, ale nie znaleziono otwartych portów w XML.[/yellow]")
                ...
                try:
                    with open(res_file, 'r') as dbg_f:
                        head = dbg_f.read(300)
                        utils.console.print(f"[dim]Początek pliku XML (debug): {head}[/dim]")
                except:
                    pass
    ...
    if discovery_tool:
        ...
        if discovery_tool == "Naabu":
            ...
            cmd = [
                ...
                "--rate",
                str(config.MASSCAN_RATE),
                "--wait", "5",  # Dodano wait, aby upewnić się, że pakiety wrócą na VPN
                "-oG",
                output_file,
            ]
            if config.EXCLUDED_PORTS:
...
def display_phase2_settings_menu(display_banner_func):
    while True:
        ...
        masscan_iface_disp = config.MASSCAN_INTERFACE if config.MASSCAN_INTERFACE else "[dim]Auto[/dim]"
