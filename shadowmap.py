...
def parse_target_input(target_input: str):
    ...
    # NOWOŚĆ: Wykrywanie i oddzielanie portu
    port = None
    # Podstawowe wykrywanie portu dla IPv4 i nazw domenowych
    ...
    port_str = f" na porcie [yellow]{config.TARGET_PORT}[/yellow]" if config.TARGET_PORT else ""
    utils.console.print(
        Align.center(f"[green]Cel: {config.HOSTNAME_TARGET}{port_str} | Domena bazowa: {config.CLEAN_DOMAIN_TARGET}[/green]")
    )
...
def generate_html_report(
    ...
    all_subdomains_content = "".join(
        read_file(f) for tool, f in p1_files.items() if tool != 'Httpx (Faza 1)'
    )
    all_subdomains_list = sorted({line.strip() for line in all_subdomains_content.splitlines() if line.strip() and "Brak danych" not in line})
    ...
    p4_json_data = {
        key: json.dumps([{"url": url} for url in data])
        for key, data in p4_results.items()
    }
    
    raw_p3_output = p3_results.get("raw_output_by_tool", {})
    ...
    replacements = {
        ...
        "{{FFUF_OUTPUT}}": raw_p3_output.get("Ffuf", "Brak danych"),
        "{{FEROXBUSTER_OUTPUT}}": raw_p3_output.get("Feroxbuster", "Brak danych"),
        "{{DIRSEARCH_P3_OUTPUT}}": raw_p3_output.get("Dirsearch", "Brak danych"),
        "{{GOBUSTER_OUTPUT}}": raw_p3_output.get("Gobuster", "Brak danych"),
        
        "{{COUNT_ALL_URLS_P4}}": len(p4_results.get("all_urls", [])),
        "{{PHASE4_ALL_URLS_JSON}}": p4_json_data.get("all_urls", "[]"),
        "{{COUNT_PARAMETERS}}": len(p4_results.get("parameters", [])),
        "{{PHASE4_PARAMETERS_JSON}}": p4_json_data.get("parameters", "[]"),
        "{{COUNT_JS_FILES}}": len(p4_results.get("js_files", [])),
        "{{PHASE4_JS_FILES_JSON}}": p4_json_data.get("js_files", "[]"),
        "{{COUNT_API_ENDPOINTS}}": len(p4_results.get("api_endpoints", [])),
        "{{PHASE4_API_ENDPOINTS_JSON}}": p4_json_data.get("api_endpoints", "[]"),
        "{{COUNT_INTERESTING_PATHS}}": len(p4_results.get("interesting_paths", [])),
        "{{PHASE4_INTERESTING_PATHS_JSON}}": p4_json_data.get("interesting_paths", "[]"),
    ...
    try:
        for current_target in targets_to_scan:
            ...
            config.REPORT_DIR = os.path.join(
                config.OUTPUT_BASE_DIR, f"report_{config.HOSTNAME_TARGET.replace(':', '_')}_{report_time}"
            )
            ...
            while True:
                ...
                if choice == "1":
                    if config.TARGET_IS_IP:
                        utils.console.print(Align.center("[bold yellow]Faza 1 jest pomijana dla celów IP.[/bold yellow]"))
                        time.sleep(2)
                    elif phase1_subdomain.display_phase1_tool_selection_menu(display_banner):
