...
def ask_scan_scope(
    ...
    summary_text = (
        ...
        f"W tym [bold red]{len(critical_results)}[/bold red] "
        "potencjalnie krytycznych."
    )
    panel = Panel(
    ...
    if not critical_results:
        utils.console.print(
            Align.center(
                "[yellow]Brak celów krytycznych. Kontynuuję ze "
                "wszystkimi znalezionymi.[/yellow]"
            )
        )
        time.sleep(1)
        return all_results
...
def display_main_menu() -> str:
    utils.console.clear()
    display_banner()
    panel = Panel.fit("[bold cyan]ShadowMap Main Menu[/bold cyan]")
    utils.console.print(Align.center(panel))
    utils.console.print(
        Align.center(
    ...
    return utils.get_single_char_input_with_prompt(prompt_text)
...
def parse_target_input(target_input: str):
    ...
    config.TARGET_IS_IP = bool(re.match(ip_pattern, clean_target))
...
def detect_waf_and_propose_safe_mode():
    ...
    text = Text("Sprawdzam ochronę WAF...", justify="center")
    utils.console.print(Align.center(Panel(text, title=panel_title)))
    try:
        ...
        if waf_name_match := re.search(waf_pattern, process.stdout):
            ...
            if utils.ask_user_decision(
                "Włączyć Tryb Bezpieczny?", ["y", "n"], "y"
            ) == "y":
...
def open_html_report(report_path: str):
    if not os.path.exists(report_path):
        msg = f"[yellow]Ostrzeżenie: Plik '{report_path}' nie istnieje.[/yellow]"
...
def generate_html_report(
    ...
    try:
        ...
        msg = f"[red]BŁĄD: Nie znaleziono szablonu: {config.HTML_TEMPLATE_PATH}[/red]"
        utils.console.print(msg)
        return None

    all_subdomains_content = "".join(read_file(f) for f in p1_files.values())
    all_subdomains_list = sorted(
        {
            line.strip()
            for line in all_subdomains_content.splitlines()
            if line.strip() and "Brak danych" not in line
        }
    )
    ...
    open_ports = p2_results.get("open_ports_by_host", {})
    open_ports_count = sum(len(p) for p in open_ports.values())
    ...
    if tech_list:
        ...
        tech_html = (
            f'<div class="tech-columns"><ul>{col1}</ul><ul>{col2}</ul></div>'
        )
    ...
    if sploit_data := p0_data.get("searchsploit_results"):
        if "Error" not in sploit_data and any(sploit_data.values()):
            ...
            for tech, exploits in sploit_data.items():
                if exploits:
                    exploit_items = "".join(
                        f'<li><a href="https://www.exploit-db.com/exploits/'
                        f'{e["id"]}" target="_blank"><span class="exploit-id">'
                        f'EDB-ID: {e["id"]}</span>{e["title"]}</a></li>'
                        for e in exploits
                    )
                    html_parts.append(
    ...
    replacements = {
        ...
        "{{OSINT_IP}}": p0_data.get("ip"),
        "{{OSINT_ASN_DETAILS}}": p0_data.get("asn_details"),
        "{{OSINT_CDN}}": p0_data.get("cdn_name"),
        "{{OSINT_REGISTRAR}}": p0_data.get("registrar"),
        ...
        "{{NAABU_RAW_OUTPUT}}": read_file(p2_results.get("naabu_file")),
        "{{MASSCAN_RAW_OUTPUT}}": read_file(p2_results.get("masscan_file")),
        ...
        "{{DIR_SEARCH_ALL_OUTPUT}}": "\n".join(
            p3_results.get("all_dirsearch_results", [])
        ),
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(template)
    utils.console.print(f"[green]✓ Raport HTML: {report_path}[/green]")
    return report_path
...
def main(
    ...
    target_list: Optional[Path] = typer.Option(
        ...
        "-l",
        "--target-list",
        help="Plik zawierający listę celów do skanowania.",
    ...
    exclude: Optional[List[str]] = typer.Option(
        ...
        "-e",
        "--exclude",
        help="Wyklucz subdomeny ze skanowania (np. 'test.domena.pl').",
    ...
    quiet_mode: bool = typer.Option(
        ...
        "-q",
        "--quiet",
        help="Tryb cichy, minimalizuje output (dla wielu celów).",
        rich_help_panel="Output",
    ),
):
    ...
    try:
        for current_target in targets_to_scan:
            ...
            p3_verified_data: List[Dict[str, Any]] = []
            p4_results: Dict[str, Any] = {}
            targets_for_phase2_3: List[str] = []
            targets_for_phase4: List[str] = []

            parse_target_input(current_target)
            report_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            report_name = f"report_{config.HOSTNAME_TARGET}_{report_time}"
            config.REPORT_DIR = os.path.join(config.OUTPUT_BASE_DIR, report_name)
            ...
            while True:
                ...
                if choice == "1":
                    menu_result = phase1_subdomain.display_phase1_tool_selection_menu
                    if menu_result(display_banner):
                        p1_files, active_urls_data, _ = phase1_subdomain.start_phase1_scan()
                        if active_urls_data:
                            all_p1 = [item["url"] for item in active_urls_data]
                            crit_p1 = utils.filter_critical_urls(all_p1)
                            targets_for_phase2_3 = ask_scan_scope(
                                all_p1, crit_p1, "Fazy 2 i 3"
                            )
                            if utils.ask_user_decision(
                                "Kontynuować do Fazy 2?", ["y", "n"], "y"
                            ) == "y":
                                choice = "2"
                                continue
                        else:
                    ...
                    if not targets_for_phase2_3:
                        msg = "[yellow]Brak celów z Fazy 1. Skanuję cel główny.[/yellow]"
                        utils.console.print(Align.center(msg))
                        targets_for_phase2_3 = [best_target_url]

                    menu_result = phase2_port_scanning.display_phase2_tool_selection_menu
                    if menu_result(display_banner):
                        p2_results = phase2_port_scanning.start_port_scan(
                            targets_for_phase2_3, None, None
                        )
                        if p2_results.get("open_ports_by_host"):
                            if utils.ask_user_decision(
                                "Kontynuować do Fazy 3?", ["y", "n"], "y"
                            ) == "y":
                                choice = "3"
                                continue
                        else:
                    ...
                    menu_result = phase3_dirsearch.display_phase3_tool_selection_menu
                    if menu_result(display_banner):
                        ...
                        with Progress(
                            ...
                            task = progress.add_task("[green]Faza 3[/green]", total=total or 1)
                            p3_results, p3_verified_data = (
                                phase3_dirsearch.start_dir_search(
                        ...
                        if p3_verified_data:
                            all_p3 = [item["url"] for item in p3_verified_data]
                            crit_p3 = utils.filter_critical_urls(all_p3)
                            targets_for_phase4 = ask_scan_scope(
                                all_p3, crit_p3, "Fazy 4"
                            )
                        else:
                            msg = "[yellow]Brak wyników z Fazy 3. Używam celów z Fazy 1.[/yellow]"
                            utils.console.print(Align.center(msg))
                            targets_for_phase4 = targets_for_phase2_3

                        if utils.ask_user_decision(
                            "Kontynuować do Fazy 4?", ["y", "n"], "y"
                        ) == "y":
                            choice = "4"
                            continue
                    choice = ""
                    ...
                    if not targets_for_phase4:
                        base = p3_verified_data or active_urls_data
                        if not base:
                            msg = "[yellow]Brak celów. Używam celu głównego.[/yellow]"
                            utils.console.print(Align.center(msg))
                            targets_for_phase4 = [best_target_url]
                        else:
                            targets_for_phase4 = [
                                item["url"] for item in base
                            ]

                    menu_result = phase4_webcrawling.display_phase4_tool_selection_menu
                    if menu_result(display_banner):
                        ...
                        with Progress(
                            ...
                            task = progress.add_task("[green]Faza 4[/green]", total=total or 1)
                            p4_results = phase4_webcrawling.start_web_crawl(
                                targets_for_phase4, progress, task
                            )
                        msg = "[bold green]Faza 4 zakończona.[/bold green]"
                        utils.console.print(Align.center(msg))
                        time.sleep(2)
                    choice = ""
