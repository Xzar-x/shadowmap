...
try:
    ...
    print(
        f"BŁĄD: Nie można zaimportować modułów z {SHARE_DIR}. Uruchom install.py. Błąd: {e}",
        file=sys.stderr,
    )
    sys.exit(1)
...
def parse_target_input(target_input: str):
    ...
    utils.console.print(
        Align.center(
            f"[green]Hostname: {config.HOSTNAME_TARGET} | Domena: {config.CLEAN_DOMAIN_TARGET}[/green]"
        )
    )
...
def detect_waf_and_propose_safe_mode():
    utils.console.print(
        Align.center(
            Panel(
                Text("Sprawdzam ochronę WAF...", justify="center"),
                title="[cyan]Detekcja WAF[/cyan]",
            )
        )
    )
        ...
        if waf_name:
            utils.console.print(
                Align.center(
                    Panel(
                        Text.from_markup(
                            f"[red]Wykryto WAF:[/red] [blue]{waf_name.group(1).strip()}[/blue]",
                            justify="center",
                        ),
            ...
            if (
                utils.ask_user_decision("Włączyć Tryb Bezpieczny?", ["y", "n"], "y")
                == "y"
            ):
                config.SAFE_MODE = True
                utils.handle_safe_mode_tor_check()  # ZMIANA: Sprawdzenie Tora od razu po włączeniu Safe Mode
        else:
            utils.console.print(
                Align.center(
                    Panel(
...
def open_html_report(report_path: str):
    if not report_path or not os.path.exists(report_path):
        utils.console.print(
            f"[yellow]Ostrzeżenie: Plik raportu '{report_path}' nie istnieje. Nie można go otworzyć.[/yellow]"
        )
        return
    ...
    try:
        if sys.platform == "darwin":
            ...
            utils.console.print(
                "[yellow]Nieznany system operacyjny. Otwórz raport ręcznie.[/yellow]"
            )
    except (FileNotFoundError, subprocess.CalledProcessError) as e:
        utils.console.print(
            f"[red]Błąd podczas otwierania raportu. Otwórz go ręcznie.[/red]\n[dim]Szczegóły: {e}[/dim]"
        )
...
def generate_html_report(
    ...
    def read_file_content(path: Optional[str]) -> str:
        if path and os.path.exists(path):
            try:
                ...
                return "Błąd odczytu"
        return ""  # ZMIANA: Zwracaj pusty string zamiast "Brak danych"
    ...
    try:
        ...
        utils.console.print(
            f"[red]BŁĄD: Nie znaleziono szablonu: {config.HTML_TEMPLATE_PATH}[/red]"
        )
        return None

    p3_results_by_tool = p3_results.get("results_by_tool", {})

    # ZMIANA: Poprawiona logika zliczania subdomen
    all_subdomains_content = "".join(read_file_content(f) for f in p1_files.values())
    ...
    count_all_subdomains = len(all_subdomains_list) if p1_files else 0

    # ZMIANA: Logika generowania listy HTML dla technologii w 2 kolumnach
    tech_list = p0_data.get("technologies", [])
    tech_html = ""
    if tech_list:
        midpoint = (len(tech_list) + 1) // 2
        col1 = tech_list[:midpoint]
        col2 = tech_list[midpoint:]

        col1_html = "".join(f"<li>{tech}</li>" for tech in col1)
        col2_html = "".join(f"<li>{tech}</li>" for tech in col2)

        tech_html = (
            f'<div class="tech-columns"><ul>{col1_html}</ul><ul>{col2_html}</ul></div>'
        )
    else:
    ...
    replacements = {
        ...
        "{{COUNT_ALL_SUBDOMAINS}}": str(count_all_subdomains),  # ZMIANA
        "{{COUNT_HTTPX}}": str(len(active_urls_data)),
        "{{COUNT_OPEN_PORTS}}": str(
            sum(len(p) for p in p2_results.get("open_ports_by_host", {}).values())
        ),
        ...
        "{{NMAP_RESULTS_RAW_JSON}}": json.dumps(
            {
                t: read_file_content(f)
                for t, f in p2_results.get("nmap_files", {}).items()
            }
        ),
        "{{NAABU_RAW_OUTPUT}}": read_file_content(p2_results.get("naabu_file")).replace(
            "`", "\\`"
        ),
        "{{MASSCAN_RAW_OUTPUT}}": read_file_content(
            p2_results.get("masscan_file")
        ).replace("`", "\\`"),
        ...
        "{{ALL_SUBDOMAINS_OUTPUT}}": "\n".join(all_subdomains_list),  # ZMIANA
    ...
    try:
        for current_target in targets_to_scan:
            ...
            config.REPORT_DIR = os.path.join(
                ...
                f"report_{config.HOSTNAME_TARGET}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}",
            )
            ...
            while True:
                ...
                if choice == "1":
                    if phase1_subdomain.display_phase1_tool_selection_menu(
                        ...
                        p1_files, active_urls_data, _ = (
                            phase1_subdomain.start_phase1_scan()
                        )
                        if active_urls_data:
                            if (
                                utils.ask_user_decision(
                                    "Zakończono Fazę 1. Kontynuować do Fazy 2 (Skanowanie Portów)?",
                                    ["y", "n"],
                                    "y",
                                )
                                == "y"
                            ...
                            utils.console.print(
                                Align.center(
                                    "[yellow]Nie znaleziono aktywnych subdomen. Wracam do menu głównego.[/yellow]"
                                )
                            )
                            time.sleep(2)
                            choice = ""
                ...
                if choice == "2":
                    if not active_urls_data:
                        utils.console.print(
                            Align.center(
                                "[bold yellow]Najpierw uruchom Fazę 1, aby znaleźć aktywne subdomeny do skanowania.[/bold yellow]"
                            )
                        )
                        time.sleep(2)
                        choice = ""
                    ...
                    if phase2_port_scanning.display_phase2_tool_selection_menu(
                        ...
                        p2_results = phase2_port_scanning.start_port_scan(
                            targets_for_p2, None, None
                        )
                        if p2_results and p2_results.get("open_ports_by_host"):
                            if (
                                utils.ask_user_decision(
                                    "Zakończono Fazę 2. Kontynuować do Fazy 3 (Wyszukiwanie Katalogów)?",
                                    ["y", "n"],
                                    "y",
                                )
                                == "y"
                            ...
                            utils.console.print(
                                Align.center(
                                    "[yellow]Nie znaleziono otwartych portów. Wracam do menu głównego.[/yellow]"
                                )
                            )
                            time.sleep(2)
                            choice = ""
                ...
                if choice == "3":
                    if phase3_dirsearch.display_phase3_tool_selection_menu(
                        ...
                        targets = [item["url"] for item in active_urls_data] or [
                        ...
                        with Progress(
                            ...
                            task = progress.add_task(
                                "[green]Faza 3: Wyszukiwanie katalogów[/green]",
                                total=total_tasks if total_tasks > 0 else 1,
                            )
                            p3_results, p3_verified_data = (
                                phase3_dirsearch.start_dir_search(
                                    targets, progress, task
                                )
                            )

                        if (
                            utils.ask_user_decision(
                                "Zakończono Fazę 3. Kontynuować do Fazy 4 (Web Crawling)?",
                                ["y", "n"],
                                "y",
                            )
                            == "y"
                ...
                if choice == "4":
                    if phase4_webcrawling.display_phase4_tool_selection_menu(
                        ...
                        targets_for_p4 = (
                            [item["url"] for item in p3_verified_data]
                            or [item["url"] for item in active_urls_data]
                            or [config.ORIGINAL_TARGET]
                        )
                        ...
                        with Progress(
                            ...
                            task = progress.add_task(
                                "[green]Faza 4: Web Crawling[/green]",
                                total=total_tasks if total_tasks > 0 else 1,
                            )
                            p4_results = phase4_webcrawling.start_web_crawl(
                                targets_for_p4, progress, task
                            )

                        utils.console.print(
                            Align.center(
                                "[bold green]Faza 4 zakończona. Wracam do menu głównego.[/bold green]"
                            )
                        )
                        time.sleep(2)
                ...
                if choice.lower() == "q":
                    report_file_path = generate_html_report(
                        p0_data,
                        p1_files,
                        active_urls_data,
                        p2_results,
